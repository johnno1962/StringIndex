...
//
//  $Id: //depot/StringIndex/Sources/StringIndex/OffsetIndex.swift#4 $
//
...
extension String {
    ...
    public enum OffsetImpl<IndexType: OffsetIndexable>: Comparable {
        ...
        public func index(in string: IndexType.StringType, from: IndexType? = nil) -> IndexType? {
            ...
            case .offsetIndex(let index, let offset):
                ...
                return index.safeIndex(offsetBy: offset, in: string)
            ...
            case .chained(let previous, let next, let offset):
                ...
                return next != nil ? next!.index(in: string, from: from) :
                        from.safeIndex(offsetBy: offset, in: string)
            }
        }
        public func locate(in string: IndexType.StringType, target: String, from: IndexType?,
            ...
            if regex {
                do {
                    ...
                    let range = IndexType.nsRange(bounds, in: string)
                    if let match = (last ? regex.matches(in: string, range: range).last :
                        regex.firstMatch(in: string, range: range))?.range {
            ...
            } else if let bounds = IndexType.toRange(from: bounds, in: string),
